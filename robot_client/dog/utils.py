...
class ThreadManager:
    ...
    def safe_thread_join(
        thread: threading.Thread, timeout: float = THREAD_JOIN_TIMEOUT
    ) -> bool:
        """
...
class CommandBuilder:
    ...
    def build_movement_command(
        self, lx: float = 0.0, ly: float = 0.0, dpadx: float = 0.0, dpady: float = 0.0
    ) -> Dict[str, Any]:
        """
    ...
    def record_action_start(self, action_name: str) -> None:
        ...
        with self._lock:
            ...
            self.stats["action_history"].append(
                {"action": action_name, "start_time": time.time(), "status": "started"}
            )
