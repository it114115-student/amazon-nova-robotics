...
"""
MQTT client for xiaoice Digital Human speech.

Subscribes to the xiaoice IoT topic and triggers adb commands
to open/close the chat UI when speech messages arrive.
"""
...
from speech_executor import SpeechExecutor
...
class SpeechPubSubClient:
    """MQTT client for xiaoice Digital Human speech commands."""

    def __init__(self, settings: Dict[str, Any], executor: SpeechExecutor):
        ...
        try:
            ...
            try:
                ...
                if action == "speech":
                    ...
                    if message:
                        logging.info("Speech command received: %s", message[:100])
                        self.executor.execute_speech(message)
                    else:
...
def main():
    try:
        ...
        # Load AWS credentials from settings, env vars, or ~/.aws/credentials
