---
name: dog
description: Publish actions to Robot dog controllers via AWS IoT.
---
...
```bash
# With named profile
./run.sh --profile my-robot-profile --robot-id dog_1 --action move_forward

# Uses 'default' profile if --profile is omitted
./run.sh --robot-id dog_1 --action move_forward
```
